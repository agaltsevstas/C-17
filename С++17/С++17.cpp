...
int main()
{
    ...
    */
    ...
    {
        std::variant<std::string, uint32_t> age; // возраст, который может быть реализован строковой датой или целочисленным значением
        age = value;

        auto ageUintPointer = std::get_if<uint32_t>(&age); // указатель на тип uint32_t
        auto ageStringPointer = std::get_if<std::string>(&age); // указатель на тип std::string
        auto ageUint = std::get<uint32_t>(age); // значение на тип uint32_t
